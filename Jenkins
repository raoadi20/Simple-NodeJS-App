        ...
        stage('Staging Deployment on Development Server') {
            steps {
                script {
                    ...
                    sh 'docker run -d --name nodejs-staging -p 3000:3000 nodejs-web-app'
                    
                    // Push the Docker image to Docker Hub
                    sh 'docker tag nodejs-web-app raoadi20/nodejs-web-app' // Tagging the image
                    sh 'docker push raoadi20/nodejs-web-app'           // Pushing the image to Docker Hub
                }
            }
        }
        ...
        stage('Deploy to Production Server') {
            steps {
                ...
                sshagent(['prod-ssh-key']) {
                    ...
                    docker pull raoadi20/nodejs-web-app
                    docker run -d --name nodejs-prod -p 3000:3000 raoadi20/nodejs-web-app
                    '
                    """
                }
            }
